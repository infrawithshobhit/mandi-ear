...
async def test_cached_price_data_accessible_offline(
    cache_manager, commodity, price, quantity, state, latitude, longitude, max_age_hours
):
    """
...
async def test_cached_commodity_prices_retrievable_offline(
    cache_manager, commodities, max_age_hours
):
    """
...
async def test_essential_data_accessible_offline(
    cache_manager, priority_manager, location_lat, location_lng, radius_km, num_mandis
):
    """
    ...
    for i in range(num_mandis):
        # Create mandis within the radius for testing
...
async def test_offline_query_caching(
    cache_manager, query_type, commodity, state
):
    """
...
@given(
    ...
    cache_size_entries=st.integers(min_value=1, max_value=20)
)
...
async def test_cache_statistics_available_offline(
    cache_manager, data_age_hours, cache_size_entries
):
    """
    ...
    for i in range(cache_size_entries):
        data = {
...
async def test_offline_data_preparation_completeness(
    cache_manager, priority_manager, location_lat, location_lng, commodities, radius_km
):
    """
